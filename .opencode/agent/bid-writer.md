...
用户真实做标书的第一步是“定模板/格式”，系统也必须遵循：

1. **招标方提供模板/附件格式**：优先以招标方模板为准（直接作为目标文档，或将表单/格式复制进目标文档）。
2. **招标方未提供模板**：选择一个“最像本项目”的历史标书/伙伴标作为格式基线，再做替换与填表。
3. **不要从空白模板开始硬写**：空白模板 + 从零生成，几乎一定会导致格式漂移/不工整/不可交付。

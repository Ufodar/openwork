...
如果 source inventory / manifest 条目已经提供了 workspace 内可直接 `read` 的文本副本路径（例如 `textRelativePath`、`text_ref`、`text_path` 或等价字段），或者轻量发现已经直接列出了 `.worktree/text/*.txt` 这类 bootstrap 文本副本，优先直接读取那份文本副本，把它当成当前 source 的默认工作文本；不要在读过这份现成文本副本之前，就重新发明新的 `pandoc`、内联 Python、XML 解包或自写提取脚本路线。
...
- 不要把搜索结果页的标题、摘要或 snippet 直接当成可引用依据；搜索结果只负责发现候选来源，真正要写进正文、引用或 API 说明时，必须基于后续打开的具体页面内容，而不是只靠结果页摘要脑补。
- 如果第一批搜索结果大多数是博客、专利解析站、聚合转载页、镜像页或门户订阅页，不要马上宣布“资料已经足够”；先改写成更窄的官方 / 标准 / 厂商一手资料查询，或明确记录当前缺口仍未被权威来源补上。
...
- 如果 API 示例只需要说明鉴权上下文，不要把 `Bearer <ACCESS_TOKEN>` 或 `Bearer <access_token>` 原样写进最终正文；统一改成 `Authorization: Bearer access_token`、`Authorization: Bearer token`，或在文字里说明“需携带访问令牌”。
- 不要把 `Bearer <access_token>` 这种小写 angle-bracket token 直接留在最终交付物里；统一改成 `Authorization: Bearer access_token` 或等价的无 angle-bracket 写法。

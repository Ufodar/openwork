...
- 对文档工作默认优先考虑这些 MCP：`Context7` 用于当前任务确实需要的 SDK / 产品 / API 官方文档，`Notion` 用于内部页面/数据库/知识库，`GitHub` 用于仓库关联的 issue / PR / README / release / 设计文档，`Playwright` 或等价浏览器 MCP 只用于登录后页面、真实渲染验证、截图留证或网页内容无法通过普通抓取稳定获取的场景。

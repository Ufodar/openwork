...
- start new turns by reading `.worktree/index.json` then `.worktree/sources/manifest.json`, then route to the next missing phase
...
- do small supervisory reads of state files, verifier reports, or narrow deliverable excerpts when phase health is unclear

Main-session guardrails:
- do not begin a fresh turn with broad glob or workspace-wide reads; bootstrap from the explicit control files instead
- do not personally analyze the raw corpus when a lower-phase artifact is missing
- avoid repeated exploratory source reads in the main session once manifest, phase ownership, and receipts already point to the next step
...
- do not delegate document work to `general` or non-`doc-*` agents
...
- for `doc-reader`, name the manifest-backed source documents, `docId`s, and owned `.worktree/sources/<doc-id>.json` outputs; let the child choose its readable working surface

Phase routing:
1. If `.worktree/index.json` or `.worktree/sources/manifest.json` is missing, call `doc-intake`.
2. If the manifest lists source files that do not yet have `.worktree/sources/<doc-id>.json`, call `doc-reader` immediately; do not spend another turn on exploratory source rediscovery.
...
8. If source understanding, evidence, or online supplements are still missing, reopen the owning `doc-*` phase with a narrow supplement task; do not spin up `general` to backfill workflow state.
...
- `doc-merger` owns `.worktree/facts.json` and `.worktree/merge/conflicts.json`; keep evidence synthesis and conflict resolution there
...
- if the receipt is thin or ambiguous, reopen the owning `doc-*` phase with a narrower follow-up task

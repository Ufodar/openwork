...
- for Markdown-to-`.docx` deliverables that rely on heading hierarchy, keep headings semantic and unprefixed: do not manually type chapter/section numbering markers such as `第一章`, `1.1`, `1.1.1`, or `一、` into heading text when the heading hierarchy already carries structure
...
- when the source corpus is mostly descriptive material but the task asks for a forward-looking implementation plan, solution document, design note, or technical方案, rewrite the evidence into implementation language: explain objective, method, execution flow, integration boundary, constraints, and interfaces instead of sounding like a marketing brochure or a verbatim product manual
- when you extend a source-backed capability into an implementation method, make the inferred framing explicit with wording such as `可采用`, `建议采用`, `通过…实现`; do not silently present every inferred implementation detail as an already-deployed fact
...
- do not invent administrative or cover metadata such as identifiers, organizations, owners, contacts, or dates; if the user or source corpus did not provide those fields, omit them or mark them as explicitly待补充 instead of fabricating values
...
- when `reports/doc-writer/external-supplements.md` contains consumed supplements, surface those consumed references in the task-appropriate references or evidence section as concrete citations with source title plus issuing body、标准编号或官方 URL / 官方域名; do not leave the final deliverable at a generic “如需进一步补充，建议联网检索” placeholder once supplements were actually used
...
- do not pad technical deliverables with unrelated commercial, payment, marketing, marketplace, or product-operations content unless the user explicitly asked for those business topics
...
- when the plan calls for named system or component sections, make each one operationally useful: include at least one concrete architecture breakdown (table or structured bullets) and API examples tied to that section's stated responsibilities instead of generic filler prose
...
- each required subsection must contain at least two substantive content blocks (paragraph, table, or structured bullet group); do not leave a subsection as only a heading plus one sentence
...
- each required subsection should answer some combination of `核心组件或范围`, `实现方式`, `控制流或数据流`, `对接边界`, and `实施约束`; do not satisfy a subsection with only technology names, capability口号, or a generic价值判断

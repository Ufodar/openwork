...
def infer_role(relative_path: str) -> str:
    """Return a generic default role.

    Semantic understanding of source roles and authority levels is
    delegated to the LLM agent (doc-intake) which enriches the
    manifest after this script runs.
    """
    return "source"
...
def infer_deliverable_format(target_doc: str | None) -> str | None:
    if not target_doc:
        return None
    suffix = Path(target_doc).suffix.lower().lstrip(".")
...
def main() -> int:
    ...
    index_payload = {
        ...
        "intent_ref": ".worktree/intent.json",
        "children": [],
    }
    intent_payload = {
        "version": 1,
        "goal": goal,
        "target_doc": target_doc,
        "deliverable_format": infer_deliverable_format(target_doc),
        "task_kind": None,
        "audience": None,
        "purpose": None,
        "sections": [],
        ...
        "external_support_requested": False,
        "notes": [],
    }
    manifest_payload = {
    ...
    write_json(worktree / "intent.json", intent_payload)

...
在写入任何应答内容之前，按以下清单逐项确认。**任何一项为"否"则停止撰写，先解决前置问题。**

```
┌─ 撰写前自检 ──────────────────────────────────────────┐
│ □ 1. index.json 的 target_doc 字段是否非 null？        │
│      → 否：回到 Stage 2 完成目标文档确定。              │
│                                                        │
│ □ 2. target_doc 指向的文件是否存在？                    │
│      → 否：检查路径是否正确，必要时重新探测。           │
│                                                        │
│ □ 3. 我即将写入的目标是 requirements.csv？              │
│      → 否：停止。你正在违反 CSV-only 规则。             │
│      → 绝不创建独立的 .md / .docx 作为内容载体。       │
│                                                        │
│ □ 4. 我是否已读取 conventions.md 全文？                 │
│      → 否：先读取，获取写作约定和决策记忆。             │
│                                                        │
│ □ 5. 当前节点是 ★ 或 # 项？                            │
│      → 是：材料搜索是否已完成？                         │
...
这个自检是程序化的——不需要"判断"，只需要逐条回答"是/否"。即使你认为某条"显然满足"，也必须逐条验证。

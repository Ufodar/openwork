...
export default function App() {
  ...
  const sidebarRefreshInFlightByWorkspaceId = new Map<string, Promise<void>>();
  const SIDEBAR_SESSION_LIMIT = 200;
  const refreshSidebarWorkspaceSessions = async (workspaceId: string) => {
    const id = workspaceId.trim();
    if (!id) return;

    const existing = sidebarRefreshInFlightByWorkspaceId.get(id);
    if (existing) {
      return existing;
    }
    ...
    const run = (async () => {
      ...
      try {
        ...
        const filtered = root ? list.filter((session) => sessionBelongsToWorkspace(root, session.directory)) : list;
    ...
    sidebarRefreshInFlightByWorkspaceId.set(id, run);
    try {
      await run;
    } finally {
      if (sidebarRefreshInFlightByWorkspaceId.get(id) === run) {
        sidebarRefreshInFlightByWorkspaceId.delete(id);
      }
    }
  };
  ...
  createEffect(() => {
    ...
    if (status === "ready") {
      ...
      const scopedSessions = activeWorkspaceRoot
        ? allSessions.filter((session) => sessionBelongsToWorkspace(activeWorkspaceRoot, session.directory))
        : allSessions;
  ...
  const parseOpenworkSessionPrefs = (openwork: Record<string, unknown>): Record<string, OpenworkSessionPrefs> => {
    ...
    for (const [sessionId, prefsValue] of Object.entries(sessions as Record<string, unknown>)) {
      ...
      const agent = normalizeStoredAgent(prefs.agent);
      if (agent) {
        prefs.agent = agent;
      } else {
        delete prefs.agent;
  ...
  const persistSessionPreferredAgent = async (sessionId: string, agent: string | null): Promise<void> => {
    const id = sessionId.trim();
    if (!id) return;
    ...
    if (nextAgent) {
      nextPrefs[id].agent = nextAgent;
    } else {
      delete nextPrefs[id].agent;
    }

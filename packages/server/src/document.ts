...
function resolveBidQcScriptPath(workspacePath: string): string {
    return join(workspacePath, ".opencode", "skills", "bid-qc", "scripts", "check_deterministic.py");
}
...
export function createDocumentRoutes(routes: unknown[], sessionWorkspaces?: SessionWorkspaceService) {
    ...
    routes.push({
        ...
        handler: async (ctx: RequestContext) => {
            ...
            if (!(await exists(scriptPath))) {
                throw new ApiError(500, "missing_dependency", "check_deterministic.py is missing in this workspace");
            }

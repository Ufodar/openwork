...
export type RuntimeKnowledgeTokenRecord = {
  tokenHash: string;
  workspaceId: string;
  sessionId: string;
  runtimeId: string;
...
type RuntimeKnowledgeTokenStore = {
  schemaVersion: 1;
  updatedAt: number;
  tokens: Record<string, RuntimeKnowledgeTokenRecord>;
};

type RuntimeKnowledgeTokenServiceOptions = {
  now?: () => number;
  defaultTtlMs?: number;
};
...
function resolveRuntimeKnowledgeTokenPath(): string {
  return join(resolveOpenworkDataDir(), "runtime-knowledge-tokens", "tokens.json");
}
...
async function readStore(path: string): Promise<RuntimeKnowledgeTokenStore> {
  if (!(await exists(path))) {
    return { schemaVersion: 1, updatedAt: Date.now(), tokens: {} };
  }
  try {
    const raw = await readFile(path, "utf8");
    const parsed = JSON.parse(raw) as Partial<RuntimeKnowledgeTokenStore>;
    const source = parsed.tokens && typeof parsed.tokens === "object" ? parsed.tokens : {};
    const tokens: Record<string, RuntimeKnowledgeTokenRecord> = {};
    for (const [tokenHashKey, value] of Object.entries(source as Record<string, unknown>)) {
      if (!value || typeof value !== "object") continue;
      const entry = value as Partial<RuntimeKnowledgeTokenRecord>;
...
async function writeStore(path: string, tokens: Record<string, RuntimeKnowledgeTokenRecord>) {
  await ensureDir(dirname(path));
  const payload: RuntimeKnowledgeTokenStore = {
    schemaVersion: 1,
    updatedAt: Date.now(),
    tokens,
  };
...
export class RuntimeKnowledgeTokenService {
  private store: RuntimeKnowledgeTokenStore | null = null;
  private readonly now: () => number;
  private readonly defaultTtlMs: number;

  constructor(options: RuntimeKnowledgeTokenServiceOptions = {}) {
  ...
  private async ensureLoaded() {
    ...
    this.store = await readStore(resolveRuntimeKnowledgeTokenPath());
    return this.store;
  }

  private async persist() {
    if (!this.store) return;
    await writeStore(resolveRuntimeKnowledgeTokenPath(), this.store.tokens);
  }

  private async pruneExpired(store: RuntimeKnowledgeTokenStore) {
    const now = this.now();
    let dirty = false;
  ...
  async issue(input: {
    ...
    const token = `owkrt_${shortId().replace(/-/g, "")}`;
  ...
  async resolve(token: string): Promise<RuntimeKnowledgeTokenRecord | null> {

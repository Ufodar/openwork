      ...
      try {
        const actor = route.auth === "host"
          ...
          : route.auth === "client"
            ...
            : undefined;
        const requestedWorkspaceId = extractWorkspaceIdFromPath(url.pathname);
        if (requestedWorkspaceId && actor?.tokenHash && actor.scope !== "owner") {
          const boundUser = await auth.getUserByOwnerKey(actor.tokenHash);
          const boundWorkspaceId = boundUser?.workspace?.id?.trim() ?? "";
          if (boundWorkspaceId && boundWorkspaceId !== requestedWorkspaceId) {
...
function extractWorkspaceIdFromPath(pathname: string): string | null {
  const mounted = parseWorkspaceMount(pathname);
  if (mounted?.workspaceId) return mounted.workspaceId;

  const direct = pathname.match(/^\/workspace\/([^/]+)/);
  if (direct?.[1]) return decodeURIComponent(direct[1]);

  const host = pathname.match(/^\/workspaces\/([^/]+)/);
  if (host?.[1]) return decodeURIComponent(host[1]);
...
async function fetchOpencodeJson(
  ...
  const directory = init.directory === undefined ? resolveOpencodeDirectory(workspace) : init.directory?.trim() ?? "";
  if (directory) {
...
async function listWorkspaceSessions(input: {
  ...
  const sharedSessionIds: string[] = [];
  const runtimeScopedEntries = new Map<string, Array<{
    sessionId: string;
    owner: { ownerKey: string; updatedAt: number };
    runtimeEntry: SessionWorkspaceEntry;
  }>>();

  for (const [sessionId, owner] of targetEntries) {
    const runtimeEntry = runtimeEntries.get(sessionId);
    const runtimeDir = runtimeEntry?.runtimeDir?.trim() ?? "";
    if (runtimeEntry && runtimeDir && resolve(runtimeDir) !== workspaceRoot) {
      const key = resolve(runtimeDir);
      const bucket = runtimeScopedEntries.get(key) ?? [];
      bucket.push({ sessionId, owner, runtimeEntry });
      runtimeScopedEntries.set(key, bucket);
      continue;
    }
    sharedSessionIds.push(sessionId);
  }
  ...
  for (const [resolvedRuntimeDir, entries] of runtimeScopedEntries) {
    const runtimeDir = entries[0]?.runtimeEntry.runtimeDir?.trim() ?? resolvedRuntimeDir;
    const runtimeWorkspace = workspaceWithDirectory(input.workspace, runtimeDir);
    let payload: unknown = null;
    try {
      payload = await fetchOpencodeJson(runtimeWorkspace, "/session", {
        method: "GET",
    ...
    const parsedById = new Map(
      normalizeSessionListPayload(payload)
        .map((value) => parseListedSession(value))
        .filter((value): value is NonNullable<typeof value> => Boolean(value))
        .map((value) => [value.id, value] as const),
    );

    for (const { sessionId, owner, runtimeEntry } of entries) {
      const runtimeProfilePreferences =
      ...
      const parsed = parsedById.get(sessionId) ?? null;
...
export async function countAdminSessionsByOwner(
  ...
  const scanned = await scanAdminSessions(config, sessionOwnership, sessionWorkspaces, ownerKey);
...
export async function proxyOpencodeRequest(input: {
  ...
  if (workspace && runtimeWorkspace && pathSessionId) {
    targetWorkspace = workspaceWithDirectory(workspace, runtimeWorkspace.runtimeDir);
  }
  if (workspace && workspaceId && !pathSessionId && requestDirectory) {
    ...
    if (matchedEntry) {
      ...
      targetWorkspace = workspaceWithDirectory(workspace, matchedRuntime.runtimeDir);
    }
  }
  if (workspace && workspaceId && requesterKey && method === "POST" && normalizedProxyPath === "/session") {
    ...
    const nextPermissions = [
      ...existingPermissions.filter((entry) => {
        ...
        return (entry as any).permission !== "external_directory";
      }),
      ...buildSessionPermissionRules(),
    ];
  ...
  try {
    ...
    if (workspaceId && !isOwnerScope && requesterKey && method === "GET" && normalizedProxyPath === "/session" && response.ok) {
      ...
      const filteredItems = items.filter((item) => {
        ...
        if (!id.trim() || !visible.has(id)) return false;
        if (!shouldScopeSessionList || !workspace) return true;
        const parsedItem = parseListedSession(item);
        return Boolean(parsedItem && sessionDirectoryBelongsToWorkspace(workspace.path, parsedItem.directory));
      });
...
async function proxyOpenCodeRouterRequest(input: {
  ...
  const method = input.request.method.toUpperCase();
  const body = method === "GET" || method === "HEAD" ? undefined : (input.request.body ?? undefined);
  try {
    const timeoutController = new AbortController();
...
async function cleanInboxViolations(inboxRoot: string, logger?: ServerLogger): Promise<number> {
  ...
  for (const v of violations) {
    try {
      ...
      logger?.log("info", `[inbox-guard] Removed unauthorized file: ${v}`);
    } catch {
...
export function upsertWorkspace(config: ServerConfig, workspace: { id: string; name: string; path: string }) {
  ...
  config.workspaces = [
    normalized,
    ...config.workspaces.filter((entry) => entry.id !== normalized.id),
  ];
  const resolvedPath = resolve(normalized.path);
...
export function createRoutes(
  ...
  addRoute(routes, "GET", "/workspaces", "client", async (ctx) => {
    const boundUser = ctx.actor?.tokenHash ? await auth.getUserByOwnerKey(ctx.actor.tokenHash) : null;
    const visible = boundUser?.workspace
      ? config.workspaces.filter((workspace) => workspace.id === boundUser.workspace?.id)
      : config.workspaces;
  ...
  const registerHandler = async (ctx: RequestContext) => {
    ...
    try {
      ...
      if (result.workspace) {
        upsertWorkspace(config, result.workspace);
      }
  ...
  addRoute(routes, "GET", "/admin/users", "host", async () => {
    const [users, scanned] = await Promise.all([
      auth.listUsers(),
      scanAdminSessions(config, sessionOwnership, sessionWorkspaces, null),
    ]);
  ...
  addRoute(routes, "GET", "/workspace/:id/inbox", "client", async (ctx) => {
    ...
    const inboxRoot = resolveInboxDir(workspace.path);
    const prefix = (ctx.url.searchParams.get("prefix") ?? "").trim();
    const items = await listInbox(inboxRoot, { prefix });
    return jsonResponse({ items });
  });
  ...
  addRoute(routes, "POST", "/workspace/:id/agentlab/automations/:automationId/run", "client", async (ctx) => {
    ...
    const provisionedRuntimeEntry = {
      runtimeId: provisionedRuntime.runtimeId,
      runtimeDir: provisionedRuntime.runtimeDir,
      createdAt: now,
    };
    const automationWorkspace = workspaceWithDirectory(workspace, provisionedRuntimeEntry.runtimeDir);
    let created: unknown;
    try {
      created = await fetchOpencodeJson(automationWorkspace, "/session", {
        ...
        body: {
          ...
          permission: buildSessionPermissionRules(),
        },
      });
    } catch (error) {
      await rm(provisionedRuntime.runtimeDir, { recursive: true, force: true }).catch(() => undefined);
    ...
    const createdSession = created && typeof created === "object" ? created as Record<string, unknown> : null;
    const sessionId =
    ...
    try {
      const promptWorkspace = automationWorkspace;

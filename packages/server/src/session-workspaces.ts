...
function buildRuntimeKnowledgeInstructions(attachedKnowledge: RuntimeKnowledgeInstructionRecord[]): string {
  const header = [
    ...
    "",
    "Use attached knowledge as the primary recall surface when it is present.",
    "",
    `Attached knowledge count: ${attachedKnowledge.length}`,
    "",
    "Rules:",
    "1. When one or more knowledge bases are attached, use `openwork_knowledge_search` before other search or factual-recall paths.",
    "2. Use `openwork_knowledge_list_attached` only when you need to confirm the current attachment set, and rerun knowledge search if the active attachments changed earlier in the conversation.",
    "3. Do not use `memory_search_nodes` or `memory_read_graph` as a substitute for attached knowledge retrieval.",
    "4. If no knowledge bases are attached, or attached knowledge is insufficient, say that briefly and then continue with other appropriate tools.",
    "",
